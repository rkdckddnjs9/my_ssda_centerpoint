    ...
    def forward(self, batch_dict):
        if self.training:
            ...
            with torch.no_grad():
                ...
                for i, ind in enumerate(unlabeled_mask):
                    ...
                    if len(unzero_inds) > 0:
                        ...
                        #                / torch.clamp((iou_max < 0.5).float().sum(dim=0, keepdim=True), min=1.0)
                        sem_score_fg = (iou_max > 0.5).float().sum(dim=0, keepdim=True) \
                                       / torch.clamp((iou_max > 0.5).float().sum(dim=0, keepdim=True), min=1.0)
                        sem_score_bg = (iou_max < 0.5).float().sum(dim=0, keepdim=True) \
                                       / torch.clamp((iou_max < 0.5).float().sum(dim=0, keepdim=True), min=1.0)
            ...
            for key in tb_dict.keys():
                if 'hm' in key:
                    hm_loss_labeled += tb_dict[key][labeled_mask, ...]
                    hm_loss_unlabeled += tb_dict[key][unlabeled_mask, ...]
                elif 'loc' in key:
                    loc_loss_labeled += tb_dict[key][labeled_mask, ...]
                    loc_loss_unlabeled += tb_dict[key][unlabeled_mask, ...]
                elif 'rpn' in key :
                    rpn_loss_labeled = tb_dict[key][labeled_mask, ...]
                    rpn_loss_unlabeled = tb_dict[key][unlabeled_mask, ...]
