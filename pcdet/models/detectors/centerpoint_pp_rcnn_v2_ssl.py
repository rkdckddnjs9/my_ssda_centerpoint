            ...
            with torch.no_grad():
                ...
                for cur_module in self.centerpoint_rcnn_ema.module_list:
                    if str(cur_module) == "BEVFeatureExtractorV2()" or str(cur_module) == "PVRCNNHead()":
                        pred_dicts, _ = self.post_processing_for_refine(batch_dict_ema)
                ...
                #pred_dicts, recall_dicts = self.centerpoint_rcnn_ema.post_processing_for_roi_ssl(batch_dict_ema,)
                pseudo_boxes = []
                pseudo_scores = []
                pseudo_labels = []
